...
const addDecarbonisationAtom = atom(null, (get, set, decarbonisation: Decarbonisation) => {
...
const removeDecarbonisationAtom = atom(null, (get, set, uuid: string) => {
  set(
    ...
    get(decarbonisationsAtom).filter((decarbonisation) => decarbonisation.uuid !== uuid),
  )
})

const hasDecarbonisationAtom = atom(null, (get, set, uuid: string) => {
  return get(decarbonisationsAtom).some((decarbonisation) => decarbonisation.uuid === uuid)
})
