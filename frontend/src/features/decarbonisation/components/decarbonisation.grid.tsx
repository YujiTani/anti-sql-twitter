...
function DecarbonisationGrid() {
  ...
  const { decarbonisations: selectedDecarbonisations } = useDecarbonisationStore()
  ...
  return (
    <div className="container relative mx-auto px-4 py-8">
      <h2 className="text-center font-bold text-2xl md:text-5xl">地球にいいこと</h2>
      <div className="mt-20 grid gap-4 md:grid-cols-3">
        {decarbonisations.map((item) =>
          item.achievements.length > 0 ? (
            <DecarbonisationCompletedItem key={item.uuid} item={item} />
          ) : (
            <DecarbonisationItem key={item.uuid} item={item} onAchieve={() => handleAchieve()} />
          ),
        )}
      </div>
    </div>
